...
const CartItem = ({ item }) => {
  const { updateItem, removeItem, toggleSelectItem, selectedItemIds } = useCartStore();
  const isSelected = selectedItemIds.includes(item.id);

  const colorAttr = item.attributeValues?.find(
    (a) => a.attributeName?.toLowerCase() === "color" || a.attributeName?.toLowerCase() === "màu sắc"
  );
  const sizeAttr = item.attributeValues?.find(
    (a) => a.attributeName?.toLowerCase() === "size" || a.attributeName?.toLowerCase() === "kích thước"
  );
    ...
    <>
      ...
      <div className="w-full flex items-center border-t py-4 gap-2">
        <label className="flex items-center gap-2 cursor-pointer select-none relative">
          ...
          />
          <div
            className={`w-5 h-5 shrink-0 border-2 rounded-sm flex items-center justify-center transition-all duration-200 ${isSelected ? "border-blue-700" : "border-neutral-500"
              }`}
          >
            <span
              className={`w-3 h-3 rounded-[1px] bg-blue-700 transition-opacity duration-200 ${isSelected ? "opacity-100" : "opacity-0"
                }`}
            />
          </div>
        </label>
        ...
        <div className="flex-1 flex md:grid md:grid-cols-4 gap-3 md:gap-4 items-center">
          {/* Ảnh sản phẩm */}
          <div className="w-20 h-20 md:w-auto md:h-auto md:col-span-1 shrink-0 rounded-xl overflow-hidden aspect-square self-start">
            <img
          ...
          <div className="col-span-2 flex-1 md:flex-initial flex flex-col gap-2 my-2 justify-between min-w-0">
            <div className="flex flex-col gap-1.5 md:gap-3">
              <div className="flex justify-between items-start gap-2 w-full">
                ...
                <div className="text-right md:hidden shrink-0">
                  ...
                  <p className="text-[10px] text-neutral-500">{formatCurrency(Number(item.price))} / cái</p>
                </div>
              </div>
              ...
              <div className="flex gap-1.5 flex-wrap">
                ...
                {!colorAttr && !sizeAttr && item.attributeValues?.map((a) => (
                  <span key={a.valueId} className="text-xs bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded-full border">
                    {a.valueName}
                  </span>
                ))}
              </div>
            ...
            <div className="flex items-center justify-between md:flex-col md:items-start md:gap-3 mt-1 md:mt-0">
              {/* Quantity control */}
              <div className="flex items-center text-xs md:text-sm border rounded-full w-fit px-1 py-0.5 bg-white">
                <button
                  onClick={() => handleQuantityChange(quantity - 1)}
                  disabled={isUpdating}
                  className="w-5 h-5 md:w-6 md:h-6 flex items-center justify-center cursor-pointer hover:bg-neutral-100 rounded-full transition-colors"
                >
                  −
                </button>
                <input
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" viewBox="0 0 24 24" className="w-3.5 h-3.5 md:w-4 md:h-4">
                  <path
                    fill="currentColor"
          ...
          <div className="hidden md:block col-start-4 text-end mr-4">
            ...
            <p className="text-xs text-neutral-500">{formatCurrency(Number(item.price))} / cái</p>
          </div>
        </div>
      </div>
    </>
