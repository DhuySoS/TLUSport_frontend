  ...
  const fetchProducts = async (pageNumber) => {
    ...
    try {
      if (user?.id) {
        ...
        const res = await aiServices.getRecommendations(user.id, 8);
        ...
        if (res?.data?.items) {
          ...
          if (res.data.items.length < 8 || res.data.currentPage >= res.data.totalPage) {
            setHasMore(false);
          }
        }
      }
      ...
      <div className="px-15">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-8">
          ...
          {loading && Array.from({ length: 4 }).map((_, index) => (
            <div key={`skeleton-${index}`} className="flex flex-col gap-4 animate-pulse">
